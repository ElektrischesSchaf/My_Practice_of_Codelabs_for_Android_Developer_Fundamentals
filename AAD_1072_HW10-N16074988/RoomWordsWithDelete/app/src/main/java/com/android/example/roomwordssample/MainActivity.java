...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        fab.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Log.d("NewWordActivity:  ","Click in fab icon" );
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == NEW_WORD_ACTIVITY_REQUEST_CODE && resultCode == RESULT_OK) {
            Log.d("Show EXTRA_REPLY",data.getStringExtra(NewWordActivity.EXTRA_REPLY));
            Log.d("Show EXTRA_ANSWER",data.getStringExtra(NewWordActivity.EXTRA_ANSWER));
            Word word = new Word(data.getStringExtra(NewWordActivity.EXTRA_REPLY), data.getStringExtra(NewWordActivity.EXTRA_ANSWER) );
            // Save the data
            mWordViewModel.insert(word);
        } else if (requestCode == UPDATE_WORD_ACTIVITY_REQUEST_CODE
                && resultCode == RESULT_OK) {

            Log.d("Implement Update","UPDATE WORD ACTIVITY");

            String word_data = data.getStringExtra(NewWordActivity.EXTRA_REPLY);
            String word_answer=data.getStringExtra(NewWordActivity.EXTRA_ANSWER);
            Log.d("Implement Update:  ",word_data+"    "+word_answer);

            int id = data.getIntExtra(NewWordActivity.EXTRA_REPLY_ID, -1);

            if (id != -1) {
                mWordViewModel.update(new Word(id, word_data, word_answer));
            } else {
    ...
    public void launchUpdateWordActivity( Word word) {
        Log.d("NewWordActivity:   ","Click recycle view");
        Intent intent = new Intent(this, NewWordActivity.class);
        intent.putExtra(EXTRA_DATA_UPDATE_WORD, word.getWord());

        // Answers
        intent.putExtra(EXTRA_DATA_UPDATE_ANSWER, word.getAnswer());
